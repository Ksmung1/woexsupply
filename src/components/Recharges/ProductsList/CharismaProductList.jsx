...
const collectionName = "charismaproductlist";

const CharismaProductList = ({ selectedItem, setSelectedItem }) => {
  const { user } = useUser();
  ...
  return (
    <div className="w-full space-y-4 text-gray-900">
      <h2 className="text-xl font-bold mb-4">Charisma Products</h2>
        ...
        {[...filteredList]
          ...
          .map((item) => {
            return (
              ...
              >
                {item.outOfStock && (
                  <div className="absolute -left-10 w-50 text-center text-[7px] font-bold bg-red-600 text-white rotate-[-45deg] py-1">
                    OUT OF STOCK
                  </div>
                )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="">
                      <div className="text-[10px] line-clamp-1 font-semibold">
                        {item.label || item.name || "Charisma Product"}
                      </div>
                    </div>
                  </div>
