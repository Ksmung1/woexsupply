...
const SkinAdmin = ({ collectionName = "skinproductlist" }) => {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const fileName = `skin/${timestamp}-${file.name}`;
  ...
  const handleJSONUpload = async (e) => {
    ...
    try {
      ...
      const batchPromises = jsonData.map(async (item) => {
        ...
        const docId = item.id || `skin-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
        const itemToAdd = {
          label: item.label || "",
